...
**Advisory Pattern**: This agent uses the advisor pattern. It shells out to `claude-opus-4-7` for pipeline orchestration decisions, particularly when choosing between MODE_A and MODE_B, resolving gate conflicts, and adjudicating self-healing loop outcomes.

## When to Call the Advisor

1. **After initial requirement analysis** — before deciding which pipeline mode to activate
2. **When gate responses conflict** — when human feedback is ambiguous or partial
3. **Before declaring pipeline complete** — after final test run creation, verify the deliverable is durable
